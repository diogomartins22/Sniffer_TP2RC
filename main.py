...
def parse():
    ...
    parser.add_argument('--iface', type=str, help= f"interface de captura {lista} Default: {conf.iface}")
    ...
    if args.log is not None:
        if args.log not in [".txt", ".csv"]:
            parser.error("Uso inválido de --log. Usa .txt, ou .csv  | Use --help for more info")

    if args.iface:
        if args.iface not in lista:
            parser.error(f"Interface inválida. Selecione uma : {lista}  | Use --help for more info") 
...
def printPacote(i, tempo, size, protocol, src_mac, dst_mac, src_ip, dst_ip, pkt, mac, info, live, qnt, iface) -> str:
    ...
    p = f"{i+1:<5}{tempo:<20} {str(iface):<10} {src:<20} {dst:<20} {protocol:<10} {size:<10} {info}"

    if live :
       print(p)

    dados = [i+1, tempo, iface, src, dst, protocol, size, info]
    return p, dados
...
def sniffer(pkt, prtl, ip, mac, log, nomeFicheiro, capture_state, live, qnt, iface) -> None:
...
def ficheiro(log) -> str:
    ...
    if log == ".txt":
        inicioT = f"----- Internet Packet Sniffer -----\n\n{'':<5}{'Tempo':<20} {"Interface":<10} {'Origem':<20} {'Destino':<20} {'Protocolo':<10} {'Tamanho':<10} {'Info'}\n"
        with open(nome, 'w') as f:
            f.write(inicioT)

    elif log == ".csv":
        inicioC = ["Tempo", "Interface", "Origem", "Destino", "Protocolo", "Tamanho", "Info"]
    ...
    try:
        if args.live:
            ...
            if not args.qnt:
                ...
                print(f"\n{'':<5}{'Tempo':<20} {"Interface":<10} {'Origem':<20} {'Destino':<20} {'Protocolo':<10} {'Tamanho':<10} {'Info'}")
                capture_state = {'i': i}
                sniff(prn=lambda pkt: sniffer(pkt, args.prtl, args.ip, args.mac, args.log, nomeFicheiro, capture_state, args.live, args.qnt, iface), store=False, stop_filter=lambda x: stop_capture['value'], iface = args.iface)
                ...
                if has_filters:
                    sniff(prn=lambda pkt: sniffer(pkt, args.prtl, args.ip, args.mac, args.log, nomeFicheiro, capture_state, args.live, args.qnt, iface), store=False, stop_filter=lambda x: stop_capture['value'] or capture_state.get('i', 0) >= args.qnt, iface=args.iface)
                else:
                    sniff(count=args.qnt, prn=lambda pkt: sniffer(pkt, args.prtl, args.ip, args.mac, args.log, nomeFicheiro, capture_state, args.live, args.qnt, iface), store=False, stop_filter=lambda x: stop_capture['value'] or capture_state.get('i', 0) >= args.qnt, iface=args.iface)
                i = capture_state.get('i', i)
